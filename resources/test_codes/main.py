...
def bc_inflow():
    ...
    if (ibcin == 2):
        m = 1.5
        for j in range(1, my):
            term = 1 / (1 + .5*(gamma-1) * m**2) ## T/To -> To = 1
            p    = term**(gamma/(gamma-1)) # isentropic relation with pressure ratio with po = 1
            rho  = p/term # isentropic relation for density with rhoo = 1
            c    = np.sqrt(gamma*p/rho)
            u    = m * c
            v    = 0.
            et   = p/(gamma-1) + .5*rho*(u**2 + v**2)
...
def rk4():
    ...
    # dt = np.ones((mx, my))*.02 -> not needed
...
# read mesh file
filename = 'ressources/test_codes/ft03.dat'
